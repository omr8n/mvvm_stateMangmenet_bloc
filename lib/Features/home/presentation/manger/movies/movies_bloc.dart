...
class MoviesBloc extends Bloc<MoviesEvent, MoviesState> {
  ...
  final MovieRepo _moviesRepository = getIt<MovieRepo>();

  Future<void> _onFetchMovies(event, emit) async {
    emit(MoviesLoadingState());
    try {
      List<MoviesGenre> genres = await _moviesRepository.fetchGenres();
      // log("genres length ${genres.length}");

      List<MovieModel> movies = await _moviesRepository.fetchMovies(page: 1);
